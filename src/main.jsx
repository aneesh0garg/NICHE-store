...
const policies = {
  ...
  refunds: ['Refund policy', 'Buyers should contact the creator with an order reference and reason for a refund. In test mode, creators can record a full refund from their dashboard. Live-payment timing depends on the payment provider.']
};
...
function OrderManager({ onClose, onRefunded }) {
  ...
  const refund = async (order) => { if (!window.confirm(`Refund ${fmt(order.amountMinor)} to ${order.buyerEmail}?`)) return; const response = await fetch(`/api/orders/${encodeURIComponent(order.id)}/refund`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ reason: 'Creator-issued test refund' }) }); const result = await response.json(); if (!response.ok) return setError(result.error || 'Could not refund this order.'); load(); onRefunded(); };
  return <div className="modal-backdrop"><section className="checkout composer orders-manager" role="dialog" aria-modal="true" aria-labelledby="orders-title"><button className="close" onClick={onClose} aria-label="Close orders">×</button><p className="eyebrow">Orders</p><h2 id="orders-title">Find a purchase.</h2><a className="export-link" href="/api/orders/export.csv">Download CSV export</a><form onSubmit={submit}><label>Search buyer email, order ID, or product<input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="buyer@example.com" /></label><button className="primary-button pay">Search</button></form>{error && <p className="form-error">{error}</p>}<div className="order-results">{orders.map((order) => <article key={order.id}><div><strong>{order.buyerEmail}</strong><p>{order.product.title} · {order.id}</p><small>{order.state === 'test_refunded' ? 'Refunded' : 'Paid'} · {fmt(order.amountMinor)}</small></div>{order.state === 'test_paid' && <button className="refund-button" onClick={() => refund(order)}>Refund</button>}</article>)}{!error && orders.length === 0 && <p>No matching orders.</p>}</div></section></div>;
}
...
function Dashboard({ onStorefront, creator, onLogout, onCreatorUpdated }) {
  ...
  const refundOrder = async (order) => { if (!window.confirm(`Refund ${fmt(order.amountMinor)} to ${order.buyerName}?`)) return; try { const response = await fetch(`/api/orders/${encodeURIComponent(order.id)}/refund`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ reason: 'Creator-issued test refund' }) }); const result = await response.json(); if (!response.ok) throw new Error(result.error || 'Could not refund this order.'); reload(); notify(`Refund recorded for ${order.buyerName}`); } catch (refundError) { notify(refundError.message); } };
  return <main className="dash-shell">
    ...
    <div className="dash-layout">
      ...
      <section className="dash-content">
        ...
        <section className="panel orders-panel"><div className="panel-head"><div><h2>Recent orders</h2><p>Your latest sales</p></div><button className="text-button" onClick={() => setOrdersOpen(true)}>View all <Arrow /></button></div><div className="orders">{(data?.recentOrders ?? []).map((order) => <Order key={order.id} initials={order.buyerName.slice(0, 2).toUpperCase()} name={order.buyerName} item={order.product?.title ?? 'Product'} price={fmt(order.amountMinor)} state={order.state} onRefund={() => refundOrder(order)} />)}</div></section></div>
...
function Order({ initials, name, item, price, state, onRefund }) { return <div className="order"><div className="order-avatar">{initials}</div><div><strong>{name}</strong><p>{item}{state === 'test_refunded' ? ' · Refunded' : ''}</p></div><span>{price}</span>{state === 'test_paid' && <button className="refund-button" onClick={onRefund}>Refund</button>}</div>; }
