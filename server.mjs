...
function serializeOrder(row) { return { id: row.id, buyerName: row.buyer_name, buyerEmail: row.buyer_email, amountMinor: row.amount_minor, discountMinor: row.discount_minor || 0, couponCode: row.coupon_code || null, currency: row.currency, state: row.state, providerPaymentId: row.provider_payment_id || null, providerOrderId: row.provider_order_id || null, refundState: row.refund_state || null, createdAt: row.created_at, product: { id: row.product_id, title: row.product_title } }; }
...
async function handleApi(req, res, url) {
  ...
  if (req.method === 'GET' && url.pathname === '/api/orders') { const creator = requireCreator(req, res); if (!creator) return; const query = String(url.searchParams.get('q') || '').trim().slice(0, 100); const pattern = `%${query.replaceAll('%', '\\%').replaceAll('_', '\\_')}%`; const orders = query ? all("SELECT orders.*, products.title AS product_title, refunds.state AS refund_state FROM orders JOIN products ON products.id = orders.product_id LEFT JOIN refunds ON refunds.order_id = orders.id WHERE orders.creator_id = ? AND (orders.id LIKE ? ESCAPE '\\' OR orders.provider_payment_id LIKE ? ESCAPE '\\' OR orders.provider_order_id LIKE ? ESCAPE '\\' OR orders.buyer_email LIKE ? ESCAPE '\\' OR orders.buyer_name LIKE ? ESCAPE '\\' OR products.title LIKE ? ESCAPE '\\') ORDER BY orders.created_at DESC LIMIT 50", creator.id, pattern, pattern, pattern, pattern, pattern, pattern) : all('SELECT orders.*, products.title AS product_title, refunds.state AS refund_state FROM orders JOIN products ON products.id = orders.product_id LEFT JOIN refunds ON refunds.order_id = orders.id WHERE orders.creator_id = ? ORDER BY orders.created_at DESC LIMIT 50', creator.id); return json(res, 200, { orders: orders.map(serializeOrder) }); }
