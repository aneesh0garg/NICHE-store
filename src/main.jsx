...
function PurchaseLibrary({ token, onBack }) {
  ...
  return <main className="auth-shell"><button className="wordmark" onClick={onBack}><span>n</span> NICHE store</button><section className="auth-card purchase-library"><p className="eyebrow">Purchase library</p><h1>Your purchases.</h1>{error && <p className="form-error">{error}</p>}{!data && !error && <p>Loading your order…</p>}{data?.purchases.map((purchase) => { const paid = ['paid', 'test_paid'].includes(purchase.state); return <article key={purchase.id}><h2>{purchase.product.title}</h2><p>{purchase.product.description}</p><small>{paid ? 'Paid' : 'Refunded'} · {fmt(purchase.amountMinor)}</small>{purchase.product.downloadUrl && paid && <><a className="primary-button purchase-link" href={purchase.product.downloadUrl}>Download {purchase.product.assetName || 'your file'}</a><small className="download-expiry">This private download link expires in 24 hours. Reopen this library to get a new one.</small></>}{purchase.product.bookingUrl && paid && <a className="primary-button purchase-link" href={purchase.product.bookingUrl} target="_blank" rel="noreferrer">Book your session</a>}{purchase.product.externalUrl && paid && <a className="primary-button purchase-link" href={purchase.product.externalUrl} target="_blank" rel="noreferrer">Open access link</a>}</article>; })}</section></main>;
}
